...
func init() {
	android.RegisterModuleType("aeon_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = aeonExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = aeonExpandVariables(ctx, dep_file)
	...
	cmd := aeonExpandVariables(ctx, String(g.properties.Cmd))
