...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^aeon_") ; then
        AEON_BUILD=$(echo -n $1 | sed -e 's/^aeon_//g')
    else
        AEON_BUILD=
    fi
    export AEON_BUILD
...
function breakfast()
{
    target=$1
    local variant=$2
    source ${ANDROID_BUILD_TOP}/vendor/aeon/vars/aosp_target_release
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the AeonOS model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch aeon_$target-$aosp_target_release-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/AeonOS-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.aeon.device | grep -q "$AEON_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $AEON_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function githubremote()
{
    ...
    git remote add github https://github.com/AeonOS-AOSP/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.aeon.device | grep -q "$AEON_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.aeon.device | grep -q "$AEON_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $AEON_FIXUP_COMMON_OUT ]; then
